...
def list_please() -> list:
    ...
    return [1, "hello", True, 3.14, ["nested", "list"]]
...
def best_letter_for_pets() -> str:
    ...
    for letter in the_alphabet:
        pets_count = len(pet_filter(letter))
...
def make_filler_text_dictionary() -> Dict:
    ...
    filler_text_dictionary = {}

    for word_length in range(3, 8):
        words_list = []
        for _ in range(4):
            url = url_base + str(word_length)
            response = requests.get(url)
            if response.status_code == 200:
                words_list.append(response.text.strip())
        filler_text_dictionary[word_length] = words_list

    return filler_text_dictionary
